...
type Role = 'user' | 'employee' | 'moderator' | 'admin';
type ManagedUser = {
	_id: string;
	name: string;
	email: string;
...
export default function UserManagement() {
	...
	const fetchUsers = useCallback(async (term = '') => {
		...
		try {
			const response = await fetch(
				`/api/users${term ? `?search=${encodeURIComponent(term)}` : ''}`,
			);
	...
	const updateRole = async (id: string, role: Role) => {
		...
		try {
			...
			setUsers((items) =>
				items.map((item) => (item._id === id ? result.data : item)),
			);
	...
	return (
		<main className='space-y-6 p-4 pt-20 lg:p-8 lg:pt-8'>
			<div>
				...
				<p className='mt-2 text-muted-foreground'>
					Manage account access across your workspace.
				</p>
			</div>
			...
			<table className='w-full min-w-[760px] text-sm'>
				<thead className='border-b bg-muted/40 text-left text-muted-foreground'>
					<tr>
						...
						<th className='p-4 text-right font-medium'>Access modification</th>
					</tr>
				</thead>
				<tbody className='divide-y'>
					{users.map((item) => {
						const self = item._id === currentUserId;
						return (
								...
								<td className='p-4 text-right'>
									<div className='flex items-center justify-end gap-2'>
										...
										<select
											...
											disabled={updating === item._id || self}
											value={item.role}
											onChange={(e) =>
												onUpdate(item._id, e.target.value as Role)
											}
											className='rounded-md border bg-background px-2 py-1.5 text-sm'
										>
											{(
												['user', 'employee', 'moderator', 'admin'] as Role[]
											).map((role) => (
